...
| `ServerRuntime` | `micron/server_runtime.py` | `ServerRuntime(config, *, agent, sessions, limiter, auth)` | Ergonomic wrapper hiding `RuntimeConfig`→`create_agent`/`SessionLogger` wiring + `RateLimiter`/`AuthPolicy` globals. Local-substitutable via `tmp_path`/`FakeClock`. |

...
| `CommandDispatcher` | `micron/tui/commands.py` | `handle(cmd) -> CommandResult` | TUI adapter wrapping `SlashCommandRegistry`. Read-only batch (`/help`, `/clear`, `/mem`, `/tools`, `/model`, `/providers`) and session-management batch (`/unload`, `/reload`, `/sessions`, `/resume`, `/last`, `/skill`, `/skills`) route through the registry; file-recovery (`/trash`, `/restore`, `/purge`, `/undo`, `/tree`) and `/exit` still through an if/elif ladder pending issue #4. Translates registry `extras` into Textual `Message` fields. |

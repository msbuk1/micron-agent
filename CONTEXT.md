...
|---|---|---|---|
| `MicronAgent` | `micron/agent.py` | `run(query) -> Iterator[Event]` (80% path); `ask(query) -> str` (run + process_events); `confirm(writes, *, query) -> Iterator[Event]` (resume after `confirmation_required`); `reconfigure/provider,model`/`set_backend`/`unload_model`/`close()` | Deep module — external seam is `MicronAgent(backend, *, config, memory, skills, tools, prompt)` (accepts dependencies). Internal module `_LoopController` (iteration, `_detect_loop`, 3-strike pivot) hides loop state. Exactly one history path (issue #28): log-backed via `derive_messages` when a session logger is active, transient in-memory (`agent._history`) when sessions are disabled (headless) — no side-channel history param, no `_HistoryCompactor`. Yields `EventType` events; `set_backend`/`reconfigure` swaps LLM via `create_backend`. Injecting `sessions` (SessionLogger with an active session) makes the log the authority: model history is derived via `derive_messages`, settled output commits as `message` entries (via `_commit`), failed/cancelled streams as log-only `attempt` entries, and `_verify_projection` asserts model-visible content is reconstructable from the log (issue #20). |
...
| `SessionLogger` | `micron/sessions.py` | `start_session / log_message / log_attempt / end_session / list_sessions / read_entries / read_session / derive_messages / get_session_context / resume_session / format_version` | Append-only JSONL log at `<context>/sessions/` — the source of truth for model history (issue #20). `log_message` commits settled model-visible content; `log_attempt` records failed/retried/cancelled streams log-only (never model-visible); `derive_messages` is the projection seam resume/fork/transcript all read through. Header carries `format_version` (v1 `turn` entries migrate forward-only at read time; committed generations never rewritten in place). `fork_session(parent_id, *, max_messages, child_id)` forks a session into a child seeded purely from the parent's `derive_messages` projection (last `max_messages` settled messages; `None` = all) — child header carries a `parent` link, attempts never carry over, parent log untouched (issue #21). |
